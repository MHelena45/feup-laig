class PieceAnimation extends CGFobject {
    ...
    calculateFrames(piecePickNumber, boardPickNumber, prologPiece){
       ...
       // this.animation.eraseTransformations();
        ...
        let x_diff = (finalPosition[0] - initialPosition[0]) / 20;
        let y_diff = (finalPosition[1] - initialPosition[1]) / 20;
        let z_diff = 10 / 20; //pice will go up 10 units
        let i;
        for(i=0 ; i < 10; i++) {
            ...
            this.animation.animations.set(i, [translate, [0,0,0], [1, 1, 1]]);
        }
        for(i=10; i <= 20; i++) {
            let translate = [initialPosition[0] + i * x_diff, 5 -(i-10) * z_diff, initialPosition[1] + i * y_diff];
            // save matrix and instance
            this.animation.instances.push(i);
            // saving transformation on map
            this.animation.animations.set(i, [translate, [0,0,0], [1, 1, 1]]);
